...
function create(node, _ref3) {
  ...
  const placeholder = getDocument().createTextNode('');
...
function create$1(node, _ref4) {
  ...
  return Object.assign({}, IfBinding, {
    ...
    placeholder: getDocument().createTextNode(''),
    template: template.createDOM(node)
  });
}
...
function textExpression(node, _ref8, value) {
  ...
  if (target.nodeType === getNode().COMMENT_NODE) {
    const textNode = getDocument().createTextNode(val);
...
function createHTMLTree(html, root) {
  const template = isTemplate(root) ? root : getDocument().createElement('template');
...
const getStyleNode = (style => {
  return () => {
    ...
    style = $(STYLE_NODE_SELECTOR)[0] || getDocument().createElement('style');
    set(style, 'type', 'text/css');
    /* istanbul ignore next */

    if (!style.parentNode) getDocument().head.appendChild(style);
    return style;
  };
})();
/**
...
function componentTemplateFactory(template, components) {
  return template(create$6, expressionTypes, bindingTypes, name => {
    return components[name] || COMPONENTS_IMPLEMENTATION_MAP$2.get(name);
  });
}
/**
...
function mountComponent(element, initialProps, componentName) {
  ...
  if (!COMPONENTS_IMPLEMENTATION_MAP$2.has(name)) panic(`The component named "${name}" was never registered`);
  const component = COMPONENTS_IMPLEMENTATION_MAP$2.get(name)({
    props: initialProps
  });
...
function register$1(name, _ref) {
  ...
  if (COMPONENTS_IMPLEMENTATION_MAP$2.has(name)) panic(`The component "${name}" was already registered`);
  COMPONENTS_IMPLEMENTATION_MAP$2.set(name, createComponent({
    name,
    css,
    template,
    exports
...
function unregister$1(name) {
  if (!COMPONENTS_IMPLEMENTATION_MAP$2.has(name)) panic(`The component "${name}" was never registered`);
  COMPONENTS_IMPLEMENTATION_MAP$2.delete(name);
...
return { __, component, install, mount, register, uninstall, unmount, unregister, version };
